...
unsigned long sleepDuration = 400;
const int ESC = 27;
...
vector<GameObject> powerupsnake;
...
unsigned int MoveThroughWalls = 0, MoveThroughSnake = 0;
bool isPowerUpWalls = false, isPowerUpSnake = false; // true if there is a PowerUp fruit; false otherwise
...
*/
		...
		FruitSymbol = RandomizeFruitSymbol();
        COORD CurrentCoordinates = GeneratingCoordinations();
		GameObject CurrentObject(CurrentCoordinates,FruitSymbol);
//		CurrentObject.Color = ColorOfFruit;
        fruit.push_back(CurrentObject);
        ...
        CurrentCoordinates = GeneratingCoordinations ();
		CurrentObject.Coordinates = CurrentCoordinates;
		CurrentObject.Symbol = PoisonSymbol;
//		CurrentObject.Color = Red;
        poison.push_back(CurrentObject);
...
void Update()
{
		...
		{
			if(snake.begin()->Coordinates.X == BorderX)
			{
				snake.begin()->Coordinates.X = WindowWidth - BorderX - 1;
				--MoveThroughWalls;
				PrintScore();
			}

			if(snake.begin()->Coordinates.X == WindowWidth - BorderX)
			{
				snake.begin()->Coordinates.X = BorderX + 1;
				--MoveThroughWalls;
				PrintScore();
			}

			if(snake.begin()->Coordinates.Y == BorderY)
			{
				snake.begin()->Coordinates.Y = WindowHeight - BorderY - 1;
				--MoveThroughWalls;
				PrintScore();
			}

			if(snake.begin()->Coordinates.Y == WindowHeight - BorderY)
			{
				snake.begin()->Coordinates.Y = BorderY + 1;
				--MoveThroughWalls;
				PrintScore();
			}
		}
		...
		for (randomAccess_iterator snakeBody = snake.begin(); snakeBody != snake.end(); ++snakeBody)
        {
				snakeBody->Symbol = '*';
				if(snakeBody == snake.begin())
				{
					snakeBody->Color = ColorOfSnake;

					if(MoveThroughSnake > 0)
					{
					...
					if(MoveThroughWalls > 0 & MoveThroughSnake > 0)
					{
						snakeBody->Color = LightBlue;
					}
				}
				else
				{
...
*/
		...
		{
			srand (time(NULL));
			if ( ( rand() % 55 ) == 0 )		// generate PowerUp fruits at random
			{
				COORD CurrentCoordinates = GeneratingCoordinations ();
		...
		if ( Score > 0 && ( isPowerUpSnake == false ) ) //generate PowerUp fruit only if player has more than 60 points and there are no other PW fruits
		{
			srand (time(NULL));
			if ( ( rand() % 13 ) == 7 )		// generate PowerUp fruits at random
			{
				COORD CurrentCoordinates = GeneratingCoordinations ();
				tail = GameObject(CurrentCoordinates.X, CurrentCoordinates.Y, PowerUpSnakeSymbol);
				powerupsnake.push_back(tail);
				tail.Color = Pink;
				tail.Draw(consoleHandle);
				isPowerUpSnake = true;
			}
		...
		for (randomAccess_iterator i = powerupsnake.begin(); i != powerupsnake.end(); ++i)
		{
				if (head.Coordinates.X == i->Coordinates.X && head.Coordinates.Y == i->Coordinates.Y)
				{
						MakeSound(happy);
						powerupsnake.erase(i);
						isPowerUpSnake= false;
						MoveThroughSnake ++;
						PrintScore();
						break;
	...
	for (randomAccess_iterator i = wall.begin(); i != wall.end(); ++i)
		{
                if (head.Coordinates.X == i->Coordinates.X && head.Coordinates.Y == i->Coordinates.Y)
				{
						...
						{
							if((head.Coordinates.X != BorderX) & (head.Coordinates.X != WindowWidth - BorderX) & (head.Coordinates.Y != BorderY) & (head.Coordinates.Y != WindowHeight - BorderY))
							{
//								MoveThroughWalls--;
								
//								wall.erase(i);
								i->Symbol = WallSymbol;
								i->Color = ColorOfWalls;
								i->Draw(consoleHandle);

								snake.begin()->Coordinates.X += direction.X;
								snake.begin()->Coordinates.Y += direction.Y;

								GameObject head1 = *snake.begin();
//								head.Color = ColorOfSnake;
								head1.Draw(consoleHandle);
...
*/
		...
		//generating borders
        for ( x = BorderX; x <= WindowWidth-BorderX; ++x)
        {
                wall.push_back(GameObject(x, BorderY, WallSymbol)); //top border
                wall.push_back(GameObject(x, WindowHeight-BorderY, WallSymbol)); //bottom border
        }
        for ( y = BorderY; y <= WindowHeight-BorderY; ++y)
        {
                wall.push_back(GameObject(BorderX, y, WallSymbol)); //left border
                wall.push_back(GameObject(WindowWidth-BorderX, y, WallSymbol)); //right border
        }

		//РіРµРЅРµСЂРёСЂР°РЅРµ РЅР° РІСЉС‚СЂРµС€РЅРёС‚Рµ СЃС‚РµРЅРё
		int n = rand() % (4000/sleepDuration);//Р±СЂРѕР№ РЅР° РІСЉС‚СЂРµС€РЅРёС‚Рµ СЃС‚РµРЅРё
		int k;              //Р±СЂРѕР№ РЅР° С‚СѓС…Р»РёС‚Рµ РІ СЃС‚РµРЅР°С‚Р°
		int p;				//РїРѕСЃРѕРєР° РІ СЃС‚РµРЅР°С‚Р°
		int l;              //Р±СЂРѕР№ С‚СѓС…Р»Рё РІ С‚РµРєСѓС‰Р°С‚Р° РїРѕСЃРѕРєР°
		bool key;	//С‚РµРєСѓС‰Р°С‚Р° С‚СѓС…Р»Р° РЅРµ СЃСЉРІРїР°РґР° СЃ РґСЂСѓРіР° С‚СѓС…Р»Р° РѕС‚ СЃС‚РµРЅР°С‚Р°
		COORD coord_brik;	//РєРѕРѕСЂРґРёРЅР°С‚Рё РЅР° С‚РµРєСѓС‰Р°С‚Р° С‚СѓС…Р»Р°

		for (int i = 0; i < n; ++i)//Р·Р° РІСЃСЏРєР° РѕС‚ РІСЉС‚СЂРµС€РЅРёС‚Рµ СЃС‚РµРЅРё
		{
				k = (rand() % 15) + 1;
				
				key = true;
				coord_brik = GeneratingCoordinations ();
				wall.push_back(GameObject(coord_brik, WallSymbol));
				...
				while (i1 < k)
				{
						...
						for (int i2 = 0; i2 < l; ++i2)
						{
								...
								for (randomAccess_iterator i3 = wall.begin(); i3 != wall.end() && key; ++i3)
                                {
										if (coord_brik.X == i3->Coordinates.X && coord_brik.Y == i3->Coordinates.Y)
										{
												key = false;
										}
								...
								for (randomAccess_iterator i3 = snake.begin(); i3 != snake.end() && key; ++i3)
								{
										if (coord_brik.X == i3->Coordinates.X && coord_brik.Y == i3->Coordinates.Y)
										{
												key = false;
...
void PrintScore()
{
		...
		current.Draw(consoleHandle);
        text = "Highest Score This Session: " +  to_string(MaxScore) + "    Score: " + to_string(Score) + "  W: " + to_string(MoveThroughWalls) + "  S: " + to_string(MoveThroughSnake);
        CenterString(text);
        current.Color = ColorOfText;
}
...
*/
        ...
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 14); //change color of text
		string txt = "Difficulty Level: ";
		switch (sleepDuration)
		{
			case 400:
				CenterString(txt + "EASY");
				break;
			case 300:
				CenterString(txt + "NORMAL");
				break;
			case 200:
				CenterString(txt + "HARD");
				break;
			case 100:
				CenterString(txt + "SUICIDE");
				break;
		}
		cout << "\n\n\n\n";
        CenterString("Select Difficulty Level:");
